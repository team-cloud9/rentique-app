/*
  @Made By: 
 */
import { likeProduct, passProduct } from "../../../services/customer-side/UserService.js";
    ...
    return `
        <div class="swipe-detail-row">
            ...
            <div class="swipe-detail-values">
                ${attrArray.map(attr => `<span class="swipe-text-tag">${attr}</span>`).join("")}
            </div>
        </div>
    `;
}
...
function renderSwipePopup() {
    ...
    swipePopupEl.querySelector("#popupImage").src = product.images?.[0] || '../../../../assets/img/no-image.png';
    swipePopupEl.querySelector(".swipe-popup-title").textContent = product.itemName || 'Unnamed Product';
    swipePopupEl.querySelector("#swipePopupBrand").textContent = product.businessName || 'Unknown Brand';
    swipePopupEl.querySelector("#swipePopupDesc").textContent = product.description || 'No description available.';
