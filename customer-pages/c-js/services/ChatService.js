...
async function getChatDataById(chatId, currentUserId) {
  ...
  if (chatSnap.exists()) {
    ...
    if (otherUserId) {
      ...
      // find from businesses
      if (!otherUser) {
        const businessRef = doc(firestore, "businesses", otherUserId);
        const businessSnap = await getDoc(businessRef);
        if (businessSnap.exists()) {
          otherUser = {
            displayName: businessSnap.data().businessName,
            profileImageUrl: businessSnap.data().brandImageUrl || "",
          };
        }
      }
