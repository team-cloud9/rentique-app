...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      console.log("[login.js] - Calling AuthService.signIn.");
      const userCredential = await authService.signIn(email, password);
      const user = userCredential.user;

      console.log(`[login.js] - User ${user.uid} signed in. Checking role...`);
      const profile = await profileService.getProfile(user.uid);

      if (window.location.pathname.includes('login-business.html')) {
        if (profile && profile.roles && profile.roles.includes('business')) {
          console.log("[login.js] - Business user confirmed. Redirecting to business dashboard.");
          window.location.href = '../../home-business/index.html'; 
        } else {
          throw { code: "auth/invalid-role" }; 
        }
      } else { 
        if (profile && profile.roles && profile.roles.includes('customer')) {
          console.log("[login.js] - Customer user confirmed. Redirecting to customer home.");
